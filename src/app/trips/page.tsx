...
export default async function TripsPage() {
  ...
  const records = await prisma.trip.findMany({ where: { OR: [{ userId: user.id }, { members: { some: { userId: user.id } } }] }, include: { stops: { include: { city: true, activities: { include: { activity: true } } }, orderBy: { order: 'asc' } }, expenses: true, members: { where: { userId: user.id }, select: { role: true } } }, orderBy: { startDate: 'desc' } });
  const trips: TripListItem[] = records.map((trip) => ({
    ...
    access: trip.userId === user.id ? 'OWNER' : (trip.members[0]?.role as 'EDITOR' | 'VIEWER'),
  }));
