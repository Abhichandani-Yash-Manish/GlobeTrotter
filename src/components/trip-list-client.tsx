        ...
        <div className="trip-card-grid">
          {filtered.map((trip) => (
            <article className="trip-card" key={trip.id}>
              <div className="trip-card-image"><ImageWithFallback src={trip.coverImage} alt={trip.name} sizes="(max-width: 760px) 100vw, 33vw" /><span className={`visibility-stamp ${trip.isPublic ? 'public' : ''}`}>{trip.isPublic ? 'PUBLIC' : 'PRIVATE'}</span></div>
              ...
              <div className="trip-card-actions"><Link href={`/trips/${trip.id}`} aria-label={`View ${trip.name}`}><Eye size={17} /></Link><Link href={`/trips/${trip.id}/edit`} aria-label={`Edit ${trip.name}`}><Edit3 size={17} /></Link><button type="button" onClick={() => togglePublish(trip)} aria-label={`${trip.isPublic ? 'Unpublish' : 'Publish'} ${trip.name}`}><Send size={17} /></button><button className="danger" type="button" onClick={() => deleteTrip(trip)} aria-label={`Delete ${trip.name}`}><Trash2 size={17} /></button></div>
            </article>
          ))}
        </div>
      )}
