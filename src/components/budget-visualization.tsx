  ...
  return (
    <div className={`budget-visuals ${compact ? 'budget-visuals-compact' : ''}`}>
      ...
      <section className="budget-chart-card daily-chart-card" aria-label="Daily spend">
        ...
        {perDay.length ? <ResponsiveContainer width="100%" height={compact ? 190 : 250} minWidth={1} minHeight={1}><BarChart data={perDay} margin={{ top: 10, right: 4, left: -22, bottom: 0 }}><CartesianGrid strokeDasharray="2 4" vertical={false} stroke="#cad9df" /><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 10 }} /><Tooltip formatter={(value) => formatMoney(Number(value))} /><Bar dataKey="spent" radius={[4, 4, 0, 0]} isAnimationActive>{perDay.map((day) => <Cell key={day.date} fill={day.over ? '#FF6B57' : '#0E7C7B'} />)}</Bar></BarChart></ResponsiveContainer> : <p className="muted-copy">Daily totals will appear as the itinerary fills in.</p>}
